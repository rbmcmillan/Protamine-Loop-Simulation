...
%     sampleDist(f,M,N,b) retruns an array X of size N of random values 
...
% NOTE FROM RBM: This program was adapted from Dmitry Savransky's version.
...
% ##    x = x(uM < f(x));
    if isempty(x)
%         # More general condition--start over if no points sampled. Works better for small N
        continue;
    end
