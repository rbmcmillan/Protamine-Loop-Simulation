function [ startSites ] = SimulateLoopFormationWithElectroBias_v11(N, a, b, d, t, theta, minDistance, maxDistance, numProt, numLoop )
% SIMULATELOOPFORMATIONWITHELECTROBIAS_v11 Given a number of segments and 
...
%   less than minDistance. The protamine closes to any end is the center of
...
%   the electrostatic multibinding model if numProt > 1.
...
for i=1:t
    ...
    %comment this out
   % midpoint = round(betarnd(2, 2, 1) * N);
    ...
    %comment this out
%     numSteps = round(betarnd(2, 2, 1) * N);
...
%     end
    ...
    leftEnd = midpoint - multiplier*(numSteps / 2);
    rightEnd = midpoint + multiplier*(numSteps / 2);
